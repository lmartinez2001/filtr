...
from models.encoders.repsurf.modules.pointnet2_utils import (
    farthest_point_sample,
    index_points,
    query_knn_point,
    query_ball_point,
)
from models.encoders.repsurf.modules.polar_utils import xyz2sphere
from models.encoders.repsurf.modules.recons_utils import (
    cal_const,
    cal_normal,
    cal_center,
    check_nan_umb,
...
def sample_and_group(
    ...
    torch.cuda.empty_cache()
    # group normal
    group_normal = index_points(
        normal, idx, cuda=cuda, is_group=True
    )  # [B, npoint, nsample, B]
    torch.cuda.empty_cache()
    # group center
    group_center = index_points(
        center, idx, cuda=cuda, is_group=True
    )  # [B, npoint, nsample, A]
    torch.cuda.empty_cache()
    ...
    if feature is not None:
        ...
        new_feature = (
            torch.cat([group_center_norm, group_normal, group_feature], dim=-1)
            if return_normal
...
def sample_and_group_all(
    center, normal, feature, return_normal=True, return_polar=False
):
    """
    Input:
    ...
    new_feature = (
        torch.cat([group_center, group_normal, feature.view(B, 1, N, -1)], dim=-1)
        if return_normal
...
def group_by_umbrella(xyz, new_xyz, k=9, cuda=False):
    ...
    torch.cuda.empty_cache()
    group_xyz = index_points(xyz, idx, cuda=cuda, is_group=True)[
        :, :, 1:
    ]  # [B, N', K-1, 3]
    torch.cuda.empty_cache()
...
class SurfaceAbstraction(nn.Module):
    ...
    def forward(self, center, normal, feature):
        ...
        if self.group_all:
            new_center, new_normal, new_feature = sample_and_group_all(
                center,
                normal,
                feature,
            ...
            new_center, new_normal, new_feature = sample_and_group(
                self.npoint,
                self.radius,
                self.nsample,
                center,
...
class UmbrellaSurfaceConstructor(nn.Module):
    ...
    def forward(self, center):
        ...
        group_xyz = group_by_umbrella(
            center, center, k=self.k, cuda=self.cuda
        ...
        if self.return_dist:
            ...
            group_normal, group_center, group_pos = check_nan_umb(
                group_normal, group_center, group_pos
            )
            new_feature = torch.cat(
                [group_center, group_polar, group_normal, group_pos], dim=-1
            )  # N+P+CP: 10
        else:
